...
export function TemplatesSection({ templates, onRefresh }: { templates: FieldTemplateItem[]; onRefresh: () => void }) {
  ...
  const [editForm, setEditForm] = useState<TemplateEditForm>({ name: "", locale: uiLocale, description: "", category: isJa ? "汎用" : "通用", root_nodes: [] });
  ...
  const handleCreate = () => {
    setIsCreating(true);
    setEditForm({ name: "", locale: uiLocale, description: "", category: isJa ? "汎用" : "通用", root_nodes: [] });
  };
  ...
  const handleEdit = (template: FieldTemplateItem) => {
    ...
    setEditForm({
      ...
      category: template.category || (isJa ? "汎用" : "通用"),
      ...
      <div className="space-y-5">
        <div className="grid grid-cols-3 gap-4">
          <FormField label={isJa ? "テンプレート名" : "模板名称"}>
            <input
              ...
              placeholder={isJa ? "例: 商品紹介テンプレート" : "如：产品介绍模板"}
          ...
          <FormField label={isJa ? "カテゴリ" : "分类"}>
            <input
              type="text"
              value={editForm.category || ""}
              onChange={(e) => setEditForm({ ...editForm, category: e.target.value })}
              placeholder={isJa ? "例: マーケティング、教育" : "如：营销、教育"}
              className="w-full px-3 py-2 bg-surface-1 border border-surface-3 rounded-lg text-zinc-200"
            />
          </FormField>
          <FormField label={isJa ? "説明" : "描述"}>
            <input
              ...
              placeholder={isJa ? "テンプレートの用途説明" : "模板用途说明"}
              className="w-full px-3 py-2 bg-surface-1 border border-surface-3 rounded-lg text-zinc-200"
            />
          </FormField>
        </div>
        ...
        <div className="border-t border-surface-3 pt-4">
          <TemplateTreeEditor
            ...
            topLevelLabel={isJa ? "テンプレート構造" : "模板结构"}
            emptyText={isJa ? "内容はまだありません。先に最上位のフェーズまたはグループを追加してください。" : "还没有添加内容，先添加顶层阶段或分组。"}
          />
        </div>
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-xl font-semibold text-zinc-100">{isJa ? "内容ブロックテンプレート" : "内容块模板"}</h2>
          <p className="text-sm text-zinc-500 mt-1">{isJa ? "再利用可能な内容ブロック構造を定義します。プロジェクト作成時に参照できます" : "定义可复用的内容块结构，创建项目时可以引用"}</p>
        </div>
        <div className="flex items-center gap-3">
          <ImportExportButtons
            typeName={isJa ? "内容ブロックテンプレート" : "内容块模板"}
      ...
      <div className="grid gap-4">
            ...
            {editingId === template.id ? renderForm() : (
              <div className="p-5 bg-surface-2 border border-surface-3 rounded-xl">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex items-center gap-2">
                    <SingleExportButton onExport={() => handleExportSingle(template.id)} title={isJa ? "このテンプレートをエクスポート" : "导出此模板"} />
                    <button onClick={() => handleEdit(template)} className="px-3 py-1 text-sm bg-surface-3 hover:bg-surface-4 rounded-lg">{isJa ? "編集" : "编辑"}</button>
                    <button onClick={() => handleDelete(template.id)} className="px-3 py-1 text-sm bg-red-600/20 text-red-400 hover:bg-red-600/30 rounded-lg">{isJa ? "削除" : "删除"}</button>
                  </div>
                </div>
              </div>
            )}
        ...
        {templates.length === 0 && !isCreating && (
          <div className="text-center py-12 text-zinc-500">
            {isJa ? "内容ブロックテンプレートはまだありません。上の「新規テンプレート」をクリックして作成してください" : "还没有内容块模板，点击上方「新建模板」创建一个"}
          </div>
        )}
      </div>
    </div>
