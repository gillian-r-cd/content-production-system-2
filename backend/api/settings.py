...
EVAL_PROMPT_PRESETS = [
    {
        ...
        "content": "你是一位真实消费者。\n\n【你的身份】\n{persona}\n\n{probe_section}\n\n请基于以下章节列表制定探索计划：\n{block_list}\n\n请严格输出 JSON（禁止额外解释）：{\"plan\":[{\"block_id\":\"id\",\"reason\":\"...\"}],\"overall_goal\":\"...\"}\n\n强约束：\n1) plan 至少 3 步（若章节少于 3 个则全部列出），不得为空。\n2) 每步必须引用有效 block_id，不得杜撰。\n3) 无法判断优先级时也必须给默认顺序。",
    },
    {
        "name": "逐块探索提示词",
        "phase": "eval_experience_per_block",
        "description": "消费体验步骤2：逐块评价",
        "content": "你是一位真实消费者。\n\n【你的身份】\n{persona}\n\n{probe_section}\n\n【之前的阅读记忆】\n{exploration_memory}\n\n【当前章节】\n{block_title}\n{block_content}\n\n请严格输出 JSON（禁止额外解释）：{\"concern_match\":\"...\",\"discovery\":\"...\",\"doubt\":\"...\",\"missing\":\"...\",\"feeling\":\"...\",\"score\":1}\n\n强约束：\n1) score 必须是 1-10 的整数。\n2) missing 必须是可执行补充项，禁止空泛评价。\n3) discovery/doubt 必须可回溯到当前章节内容。",
    },
    {
        "name": "探索总结提示词",
        "phase": "eval_experience_summary",
        "description": "消费体验步骤3：综合总结",
        "content": "你是一位真实消费者。\n\n【你的身份】\n{persona}\n\n{probe_section}\n\n以下是逐块结果：\n{all_block_results}\n\n请严格输出 JSON（禁止额外解释）：{\"overall_impression\":\"...\",\"concerns_addressed\":[],\"concerns_unaddressed\":[],\"would_recommend\":false,\"summary\":\"...\"}\n\n强约束：\n1) concerns_addressed / concerns_unaddressed 每一项都要有逐块证据。\n2) summary 必须包含“是否推荐 + 条件/原因”。\n3) 信息不足时必须明确写入 concerns_unaddressed。",
    },
    {
        "name": "场景角色A提示词",
        "phase": "eval_scenario_role_a",
        "description": "场景模拟角色A模板",
        "content": "{persona}\n\n你掌握以下内容：\n{content}\n\n{probe_section}\n\n规则：\n1) 每次回复不超过50字。\n2) 仅基于内容回答，不编造信息。\n3) 若内容未覆盖，必须明确说“内容未覆盖”。\n4) 每轮推进一个决策节点（需求/异议/价值映射/决定）。",
    },
    {
        "name": "场景角色B提示词",
