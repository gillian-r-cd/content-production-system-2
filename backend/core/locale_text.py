...
- 若一个单元格需要多条内容，用 <br> 换行，不要增加 | 列分隔符""",
        ...
        "block.markdown_tail": "\n\n---\n{instructions}",
        "block.extra_instruction_header": "\n\n---\n# 额外指令\n{instruction}",
        "block.generate.human": "请生成「{name}」的内容。",
        "block.dependencies_missing_content": "以下依赖内容尚未完成: {missing_labels}",
        "block.dependencies_not_ready": "以下依赖内容尚未就绪: {missing_labels}",
        "block.confirm.stale": "内容块「{name}」的依赖已更新，当前内容已过期，请先重新生成或手动更新后再确认。",
        "block.dependency_update.manual_attention": "您修改了「{name}」的内容。以下下游内容块已标记为待更新，仍需要人工处理：{affected_names}",
...
3. 总体评价""",
        ...
        "agent.reference_context.none": "（无）",
        "agent.reference_context.bool_yes": "是",
        "agent.reference_context.bool_no": "否",
        "agent.reference_context.depends_on": "[依赖内容块]",
        "agent.reference_context.pre_questions": "[生成前提问]",
        "agent.reference_context.pre_question_required": "必答",
        "agent.reference_context.pre_question_optional": "选答",
        "agent.reference_context.pre_answers": "[当前回答]",
        "agent.reference_context.need_review": "[需要人工确认: {value}]",
        "agent.reference_context.auto_generate": "[自动生成: {value}]",
        "agent.reference_context.needs_regeneration": "[待重新生成: {value}]",
        "agent.reference_context.model_override": "[模型覆盖: {value}]",
        "agent.reference_context.default_model": "默认模型",
        "agent.reference_context.status": "[状态: {status}]",
        "agent.reference_context.block_type": "[块类型: {block_type}]",
        "agent.reference_context.special_handler": "[特殊处理器: {special_handler}]",
        ...
        "agent.references.full_block_context": "该内容块当前运行时信息：",
        ...
        "version.not_found": "未找到指定版本",
        "version.entity_not_found": "未找到对应内容块",
        "version.rollback.success": "已回滚到版本 v{version}",
        "project.markdown_import.project_missing": "项目不存在",
        "project.markdown_import.empty_files": "未提供可导入的 Markdown 文件",
        "project.markdown_import.invalid_mode": "不支持的 Markdown 导入模式：{mode}",
        "project.markdown_import.invalid_file_payload": "Markdown 导入失败：{file_label} 不是有效文件对象",
        "project.markdown_import.invalid_file_name": "Markdown 导入失败：{file_label} 缺少有效文件名",
        "project.markdown_import.invalid_file_content": "Markdown 导入失败：{file_label} 的 content 必须是字符串",
        "project.markdown_import.intro_node": "导言",
        "project.markdown_import.default_file_name": "未命名文件",
        "project.markdown_import.fallback_to_raw_file": "文件「{file_label}」未检测到 heading，已自动回退为 raw_file 导入。",
        "project.markdown_import.success": "已导入 {file_count} 个 Markdown 文件，新增 {blocks_created} 个内容块",
...
请直接输出修改后的完整内容，不要添加任何解释或前缀。""",
        ...
        "agent.query.system": "你是内容分析助手。以下是内容块「{target_label}」的当前运行时信息（含正文与可见配置）：\n\n{content}",
...
- 1 つのセルに複数項目を入れる場合は <br> で改行し、| を増やさない""",
        ...
        "block.dependencies_not_ready": "以下の依存コンテンツが未準備です: {missing_labels}",
        "block.confirm.stale": "内容ブロック「{name}」の依存関係が更新されました。現在の内容は古くなっているため、再生成または手動更新の後に確認してください。",
        "block.dependency_update.manual_attention": "「{name}」の内容を更新しました。以下の下流内容ブロックは更新待ちとしてマークされており、引き続き手動対応が必要です: {affected_names}",
...
3. 総合評価""",
        ...
        "agent.reference_context.none": "（なし）",
        "agent.reference_context.bool_yes": "はい",
        "agent.reference_context.bool_no": "いいえ",
        "agent.reference_context.depends_on": "[依存コンテンツ]",
        "agent.reference_context.pre_questions": "[事前質問]",
        "agent.reference_context.pre_question_required": "必須",
        "agent.reference_context.pre_question_optional": "任意",
        "agent.reference_context.pre_answers": "[現在の回答]",
        "agent.reference_context.need_review": "[手動確認が必要: {value}]",
        "agent.reference_context.auto_generate": "[自動生成: {value}]",
        "agent.reference_context.needs_regeneration": "[再生成待ち: {value}]",
        "agent.reference_context.model_override": "[モデル上書き: {value}]",
        "agent.reference_context.default_model": "既定モデル",
        "agent.reference_context.status": "[ステータス: {status}]",
        "agent.reference_context.block_type": "[ブロック種別: {block_type}]",
        "agent.reference_context.special_handler": "[特殊ハンドラ: {special_handler}]",
        ...
        "agent.references.full_block_context": "この内容ブロックの現在のランタイム情報:",
        ...
        "project.markdown_import.project_missing": "プロジェクトが存在しません",
        "project.markdown_import.empty_files": "インポートする Markdown ファイルがありません",
        "project.markdown_import.invalid_mode": "未対応の Markdown インポートモードです: {mode}",
        "project.markdown_import.invalid_file_payload": "Markdown インポートに失敗しました: {file_label} が有効なファイルオブジェクトではありません",
        "project.markdown_import.invalid_file_name": "Markdown インポートに失敗しました: {file_label} に有効なファイル名がありません",
        "project.markdown_import.invalid_file_content": "Markdown インポートに失敗しました: {file_label} の content は文字列である必要があります",
        "project.markdown_import.intro_node": "前書き",
        "project.markdown_import.default_file_name": "名称未設定ファイル",
        "project.markdown_import.fallback_to_raw_file": "ファイル「{file_label}」に heading が見つからなかったため、raw_file として取り込みました。",
        "project.markdown_import.success": "{file_count} 個の Markdown ファイルを取り込み、{blocks_created} 個の内容ブロックを追加しました",
...
修正後の完全版のみを出力し、解説や前置きは付けないでください。""",
        ...
        "agent.query.system": "あなたはコンテンツ分析アシスタントです。以下はコンテンツブロック「{target_label}」の現在のランタイム情報（本文と可視設定を含む）です。\n\n{content}",
        "version.not_found": "指定したバージョンが見つかりません",
        "version.entity_not_found": "対象の内容ブロックが見つかりません",
        "version.rollback.success": "バージョン v{version} にロールバックしました",
