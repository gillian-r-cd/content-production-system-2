...
| Eval Report 诊断 | `frontend/components/eval-phase-panel.tsx` | ✅ 已实现 + M3 已桥接 | 新报告 Tab 已在两个位置提供 [🤖 让Agent修改] 按钮（Trial 级 Grader 建议 + Task 级跨 Trial 分析），复用 `onSendToAgent` 通信链路（详见 `eval_v2_redesign.md` 5.4a 节） |
...
### 3.5 通路 B（Eval Report）的处理：独立展示 + Agent 桥接

> 参见 `eval_v2_redesign.md` 第 5.4a 节的完整桥接设计。

Eval 2 报告产生的评估结果在报告 Tab 中独立展示，**不经过 Agent Panel**。但报告中的**可操作建议**可以一键桥接到 Agent 面板。

Eval 2 报告有两个层级出现 [🤖 让Agent修改] 按钮：
1. **Trial 级 Grader 建议**（每个 Trial 展开后的 Grader feedback 中）：单次评估的具体改进建议
2. **Task 级跨 Trial 分析建议**（Task 看板底部 [🔍 生成跨 Trial 分析] 的输出中）：多次 Trial 的共性发现 + 逐条修改建议

> **注意**：不再有"综合诊断"（跨 Task 分析），因为跨 Task 内容可能已变更，聚合无意义。

```
Eval 2 报告 Tab                     Agent Panel
┌──────────────────────────┐       ┌────────────────┐
│ Task: 消费者体验测试        │       │                │
│  ┌ Trial 1: 张晨           │       │                │
│  │  Grader 建议:            │       │ [用户消息]      │
...
│  └                          │       │                │
│                              │       │                │
│  跨 Trial 分析               │       │ [Suggestion Card]│
│  "3次测试均出现性价比疑虑"    │       │  Agent 修改提案  │
│  [🤖 让Agent修改] ───────────────→ │                │
└──────────────────────────┘       └────────────────┘
```
...
3. **桥接逻辑极简**：只是往 Agent 对话里发一条带上下文的消息
4. **不增加新 API**：复用已有的 `onSendToAgent` prop 透传链
...
| Eval Report | **已桥接 + 已迁移**。新报告 Tab 已在 Trial 级 Grader 建议与 Task 级跨 Trial 分析建议处提供 [🤖 让Agent修改] 按钮（M3 T3.4 完成）。详见 `eval_v2_redesign.md` 5.4a 节 | 小改（复用通信链路） |
...
**目标**：Eval 2 报告中的评估建议可以一键发送到 Agent 对话，触发 Suggestion Card。

> 注：Eval 2 重构（见 `eval_v2_redesign.md`）会重建报告面板。M3 的桥接机制需在重建后的面板中重新实现。
> 通信链路（`onSendToAgent` prop 透传）和 Agent 侧逻辑（自动走对话流触发 `propose_edit`）已在 M3 首次实现中验证可行，新面板复用同一架构。

**范围**：
- 前端：报告 Tab 中两个位置添加 [🤖 让Agent修改] 按钮：
  - Trial 级 Grader 建议（每个 Trial 展开后的 feedback 中）
  - Task 级跨 Trial 分析建议（Task 看板底部 [🔍 生成跨 Trial 分析] 的输出中）
- 消息构造：包含建议标题 + 描述 + 改进方向 + 来源（Task + Trial 信息）
- 后端：无需改动（按钮只是往 Agent 对话发一条消息）
- Prompt：在消息中附带评估上下文（建议来自哪个 Task + Trial + Grader）

**验收标准**：
1. Trial 级 Grader 的改进建议旁有 [🤖 让Agent修改] 按钮
2. Task 级跨 Trial 分析的逐条建议旁有 [🤖 让Agent修改] 按钮
3. 点击后 Agent Panel 自动接收消息并生成 Suggestion Card
4. 消息内容包含足够上下文（建议描述 + 改进方向 + 来源信息），Agent 能据此生成有针对性的修改提案
...
> **状态说明**：T3.1-T3.3 已在旧 `EvalReportPanel` 上完成首次实现。
> Eval 2 重构（`eval_v2_redesign.md`）会重建报告面板，届时需在新面板中重新实现桥接按钮。
> 通信链路（`onSendToAgent` → `WorkspacePage` → `AgentPanel`）和 Agent 侧逻辑已验证可行，可直接复用。

- [x] **T3.1** 在旧 `EvalReportPanel` 的综合诊断区域添加"让 Agent 修改"按钮（已完成，待 Eval 2 重建时迁移）
  - `eval-field-editors.tsx`: `EvalFieldProps` 增加 `onSendToAgent` 可选 prop，`EvalReportPanel` 诊断区渲染按钮
- [x] **T3.2** 点击按钮后，构造消息文本（包含诊断内容）发送到 Agent Panel（已完成，通信链路可复用）
...
- [x] **T3.3** Agent Panel 接收消息后正常走 Agent 对话流（自然触发 propose_edit）（已完成，无需改动）
  - `agent-panel.tsx`: 新增 `externalMessage` + `onExternalMessageConsumed` props；useEffect 监听 externalMessage 变化后自动调用 handleSend
- [x] **T3.4** Eval 2 重建后：在新报告 Tab 的两个位置添加 [🤖 让Agent修改] 按钮
  - Trial 级 Grader 建议（每个 Trial 展开后的 feedback 中，每条改进建议旁）
  - Task 级跨 Trial 分析建议（Task 看板底部 [🔍 生成跨 Trial 分析] 输出的逐条建议旁）
  - 复用 T3.2 的 `onSendToAgent` 通信链路
  - 消息构造：包含建议标题 + 描述 + 改进方向 + 来源（Task 名 + Trial 索引 + Grader 名）
...
| **缺陷 B: 评估→迭代没有闭环** | M3 (Eval Report 桥接) 让评估结果直接触发 Agent 的 propose_edit。Eval 2 重构后，报告 Tab 中的 Trial 级 Grader 建议和 Task 级跨 Trial 分析建议均附 [🤖 让Agent修改] 按钮（详见 `eval_v2_redesign.md` 5.4a 节） |
