...
> 2026-02-19 补充：评估子系统已切换到 Eval V2（详见 `eval_v2_redesign.md`）。
...
#### 决策 6：评估是结构化的（Eval V2: Task 容器 + TrialConfig）

不是让 LLM 写一段"综合评价"，而是：
- **EvalTaskV2**：任务容器（按评估目标组织）
- **EvalTrialConfigV2**：最小配置单元（每个 Trial 可独立 form_type）
- **EvalTrialResultV2**：执行结果（含完整 LLM 调用日志）
- **TaskAnalysisV2**：Task 级跨 Trial 模式分析（可选）
- **Grader**：独立一等公民评分器
